...
export default function AllLinksPage() {
  ...
  useEffect(() => {
    ...
    const filtered = links.filter(
      (linkItem) =>
        linkItem.slug.toLowerCase().includes(query) ||
        linkItem.title?.toLowerCase().includes(query) ||
        linkItem.description?.toLowerCase().includes(query) ||
        linkItem.destinationUrl.toLowerCase().includes(query) ||
        linkItem.organization.name.toLowerCase().includes(query),
    );
            ...
            {deletingLink && (
              <div className="border border-[#e8e6df] bg-[#faf9f6] p-4">
                ...
                <p className="font-body text-xs text-muted-foreground mt-2">
                  from {deletingLink.organization.name}
                </p>
              </div>
            )}
          </div>
